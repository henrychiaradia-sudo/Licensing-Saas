    ...
    <div className="mx-auto max-w-3xl">
      <div className="mb-5">
        <h1 className="flex items-center gap-2 text-xl font-bold">
          <UserCheck size={20} className="text-blue-500" /> Meus Dados
        </h1>
      ...
      {data.vinculo && (
        <Card className="mb-4 p-5">
          <h2 className="mb-3 flex items-center gap-2 text-sm font-semibold">
            <Building2 size={16} className="text-blue-500" /> Empresa vinculada ({String(data.vinculo.tipo)})
          </h2>
          <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm sm:grid-cols-3">
      ...
      <Card className="mb-4 flex flex-wrap items-center justify-between gap-3 p-5">
        <div>
          <h2 className="flex items-center gap-2 text-sm font-semibold">
            <Download size={16} className="text-blue-500" /> Portabilidade
          </h2>
      ...
      <Card className="p-5">
        ...
        <div className="flex flex-wrap items-center gap-4 text-sm">
          <a
            href={`mailto:${DPO.email}?subject=${encodeURIComponent("Solicitação LGPD — titular de dados")}`}
            className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 font-medium text-white hover:bg-blue-700"
          >
            <Mail size={15} /> Solicitar eliminação/correção ao encarregado
          </a>
